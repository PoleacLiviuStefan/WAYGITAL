...
const Steps = () => {
  ...
  return (
    <div name="Etape De Lucru" className='relative flex flex-col items-center w-full h-[97rem] lg:h-[65rem] bg-white'>
        ...
        <div className='relative flex lg:flex-row flex-col  items-center  justify-center w-[65rem]  '>
            ...
            <div className='h-full w-full '>
                <div className='absolute lg:hidden bottom-[-43rem] z-30   bg-gradient-to-t from-white to-transparent bg-opacity-[50%] h-[15rem] w-full   ' />
        </div>  
        </div>
        <div className='relative flex justify-center items-center w-[90%] lg:w-[700px] h-[450px] lg:h-[350px] mt-[53rem] lg:mt-[25rem]  rounded-[16px] '>
        <span onClick={()=>{if(stepText>0) setStepText(prev=>prev-=1); if(stepText==3) setStepsGroup(0)}} className="absolute  top-[13rem] lg:top-[10rem] z-40 left-0 text-[48px] lg:text-[56px] rotate-[180deg]  text-[#9E00FF] cursor-pointer"><BsChevronRight /></span>
            <span onClick={()=>{if(stepText<5) setStepText(prev=>prev+=1); if(stepText==2) setStepsGroup(1)}}   className="absolute  top-[13rem] lg:top-[10rem]  z-40 right-0 text-[48px] lg:text-[56px] rotate-[0deg] text-[#9E00FF] cursor-pointer"><BsChevronRight /></span>
            ...
            <div className='absolute flex flex-col items-center justify-center  rounded-[16px] bg-[#1A1A1A] w-full h-full shadow-[10px_10px_39px_-5px_rgba(0,0,0,0.67)]'>
                <h3 className='absolute top-[3rem] text-[18px] text-center lg:text-[28px] text-white font-bold '>{stepText ==0 ? "Planificarea Proiectului": stepText==1 ? "Crearea Design-ului": stepText===2 ?"Dezvoltarea site-ului":stepText==3 ? "Testare si Lansare": stepText==4 ? "Imbunatatiri Periodice": "Optimizare pentru Motoarele de Cautare"}</h3>
